...
const JobApply = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const payload = {
        ...
        job_id: id === "general" ? null : (id || null),
        job_title: job?.title || "General Application",
      };

      const response = await apiClient.post("/public/jobs/apply", payload);
  ...
  if (submitted) return (
    <Layout>
      <PageHero title="Application" highlight="Submitted" subtitle="Your application has been received successfully!" />
      <section className="py-20 bg-background">
        <div className="container mx-auto px-4 max-w-lg">
          <div className="bg-card rounded-2xl border border-border p-8 text-center space-y-6">
            ...
            <h2 className="text-2xl font-bold text-card-foreground">Application Submitted Successfully!</h2>
  ...
  return (
    <Layout>
      <PageHero title="Apply for" highlight={job?.title || "Position"} subtitle={job ? `${job.department} · ${job.location}` : "Submit your application"} />
          ...
          <form onSubmit={handleSubmit} className="bg-card rounded-2xl border border-border p-8 space-y-8">
            ...
            <div className="flex flex-wrap gap-4 items-center bg-secondary/50 rounded-xl p-4">
              <div>
                ...
                <p className="font-bold text-card-foreground">{job?.title || "General Application"}</p>
              </div>
              {job && (
                <div>
                  <p className="text-xs text-muted-foreground">Job ID</p>
                  <p className="font-mono text-sm text-accent font-semibold">{("id_code" in job && job.id_code) || id}</p>
                </div>
              )}
