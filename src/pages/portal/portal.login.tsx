...
function RegisterModal({ 
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      await apiClient.post("/portal/auth", {
        action: "register",
        payload: {
          email: form.email,
...
export default function PortalLoginPage() {
    ...
    try {
      ...
      if (!hasClientKey) {
        ...
        if (userId) {
          setGenerating(true);
          try {
            const res = await apiClient.post("/portal/auth", {
              action: "generateKey",
              payload: { userId: loggedUserId }
            });
  ...
  const handleVerifyKey = async (e: React.FormEvent) => {
    ...
    try {
      const res = await apiClient.post("/portal/auth", {
        action: "verifyKey",
        payload: { userId: loggedUserId, key: securityKeyInput.trim() }
      });
      if (res.data.valid) {
        if (keepSigned) {
          localStorage.setItem(TRUST_KEY(email.trim()), "1");
